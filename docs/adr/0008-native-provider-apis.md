...
PROJECT.md committed to open protocols — IMAP, CalDAV, CardDAV — to _"generalize across Gmail/Fastmail/iCloud/self-hosted mail rather than locking to one vendor's OAuth quirks."_
...
**The `Connect` flow is identical across tiers.** The user clicks _Connect Google_, consents, and Ledgr stores the tokens. Only the source of the client ID changes. When the security assessment is passed, shipping default credentials is a configuration default — no user-facing change, no migration, no rewrite.
...
- Model the `Connect` UI around providers being _available_ or _not available_, never around "you must first create a Google Cloud project". That message belongs in a setup guide, surfaced only when the provider is unconfigured.

## Known friction, so it isn't a surprise later

- **Gmail mail scopes are _restricted_.** Production use by a published app requires annual independent security assessment (five figures). Planned, not yet held.
